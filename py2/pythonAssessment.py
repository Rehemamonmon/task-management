...
def count_paragraphs(text):
    if not text.strip():  # Edge case: empty string
        return 1
    paragraphs = text.split('\n\n')  # Split by two newlines
    return len([p for p in paragraphs if p.strip()])  # Exclude empty strings
...
The Apple Pie Master from ACME Inc. represents a significant advancement in the field of culinary technology. By automating the process of baking apple pies, this machine not only makes baking more accessible but also sets a new standard for the integration of technology in traditional cooking practices. As more consumers and businesses adopt this technology, it could well redefine our cooking experiences and expectations.
    ...
    specific_word = "fox"  # The word you want to count

    print("Count of specific word:", count_specific_word(text, specific_word))
    print("Most common word:", identify_most_common_word(text))
    print("Average word length:", calculate_average_word_length(text))
    print("Number of paragraphs:", count_paragraphs(text))
